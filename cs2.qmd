...
```{r}
# Function to get proportion of dark pixels in the grid
...
calc_prop <- function(region, row) {
  ...
  pos <- (region == 1)
  dark_pixels <- digit_mat[pos] > 50  # Threshold for darkness
...
# Plot to see if dark pixels is statistically discernible across digits
...
```{r}
# Cross validation to get the right params
set.seed(11012024)
...
```{r}
weekly_roc = roc_curve(bind_cols(weekly_preds=rf_pred_best$.pred_2,real_class=rf_pred_best$digit),
                       real_class,
                       weekly_preds,
                       event_level = "first")
autoplot(weekly_roc)
...
vip::vip(rf_fit_best)
```
Most important variables: V346, V298, 244, V348, V376, V270, V271, V375, V272, V374

```{r}
# Function to plot the 'important' pixels
plot_digit2 <- function(row,  highlight_pixels = c(346, 298, 244, 348, 376, 270, 271, 375, 272, 374)) {
...
```{r}
misclassified_records_final <- rf_pred_best |>
  filter(.pred_class != digit)

misclassified_records_final
```


```{r}
plot_digit(misclassified_records_final[4,])
```
KNN: 
