...
```{r}
## Tester Code
plot_region <- function(mnist_tbl) {
  digit_mat <- as.matrix(mnist_tbl) * 128 # Convert tbl into matrix and assign gray=128
...
calc_prop(region2, mnist_tbl[1, 2:785])

calc_prop(region2, mnist_tbl[5, 2:785])

calc_prop(region2, mnist_tbl[13,2:785])
...
Next steps: Define regions to help define each digit.
...
mnist_test_val_split <- initial_split(mnist_tbl, prop = 0.15, strata = digit)
...
## random forest?
rf_spec <- 
  rand_forest(trees = 100, mtry = 4) |>            
  set_mode("classification") |>                        
  set_engine("ranger", importance = "impurity")

rf_model <- rf_spec |> 
  fit(digit ~ ., data = mnist_train)

rf_model
```
```{r}
rf_oob_rmse <- rf_model$fit$prediction.error
cat("Out-Of-Bag (OOB) RMSE:", rf_oob_rmse, "\n")

mnist_train_preds <- predict(rf_model, new_data = mnist_train)$.pred_class

# Calculate Accuracy
train_accuracy <- accuracy_vec(truth = mnist_train$digit, estimate = mnist_train_preds)
cat("Training Accuracy:", train_accuracy, "\n")
```
```{r}
confusionMatrix(data = mnist_train_preds, reference = mnist_train$digit)
```

```{r}
#cv cross validation 10 folds
set.seed(11012024)
cv_folds <- vfold_cv(mnist_train, v = 10, strata = digit)
...
rf_results <- rf_workflow |>
  fit_resamples(
    resamples = cv_folds,
    metrics = metric_set(roc_auc), ##also accuracy and other metrics
